...
type Props = { scope: Scope; ownerId: string; item: ExecutionSurfaceItem; onBack: () => void };

/** Rendering is selected from capability metadata; the overlay owns no viewer-specific branch. */
export default function ExecutionSurfaceViewport({ scope, ownerId, item, onBack }: Props) {
  ...
  if (shouldUseNativeExecutionSurface(item, Boolean(window.desktop))) return <NativeObservationView {...props} />;
