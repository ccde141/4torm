...
import { streamChatCompletion, getAllModels, getProviderForModel } from '../../llm';
import { getToolsByNames, type ToolDef } from '../../store/tools';
import { readSkillToolDefs } from '../../store/skills';
import { readText } from '../../api/storage';
...
import { buildWorkspaceInfo } from '../../engine/chat/context';
...
export default function ChatPage({ preselectSession, onClearPreselect }: { preselectSession?: string; onClearPreselect?: () => void }) {
  ...
  const {
    ...
    compactSession,
    setActiveSessionId,
  } = useSessionList(selectedAgent, selectedModel, models, setSelectedAgent, setMessages, setPermissions, setStreaming, setSelectedModel);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const ws = agent?.config?.workspace || `data/agents/${selectedAgent.id}/.workspace/`;
      systemText += buildWorkspaceInfo(ws);
      ...
      await runStreamLoop({
        session, allMessages: updatedMessages, chatMessages, loopCount, maxLoops,
      ...
      <div style={leftPanelStyle}>
        <div style={{ marginBottom: 'var(--space-4)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            {agents.map(agent => (
              <button key={agent.id} onClick={() => selectAgent(agent)} style={{ ...agentBtnStyle, background: selectedAgent?.id === agent.id ? 'var(--color-accent)' : 'transparent', color: selectedAgent?.id === agent.id ? 'var(--color-text-inverse)' : 'var(--color-text-secondary)', fontWeight: selectedAgent?.id === agent.id ? 'var(--font-semibold)' : 'var(--font-normal)' }}>
