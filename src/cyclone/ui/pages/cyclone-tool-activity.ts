const SYSTEM_TOOLS = new Set([
  'ask', 'delegate', 'contact', 'dispatch', 'bulletin', 'task_board', 'use_skill',
  'register_tool', 'register_skill', 'review_changes', 'list_agents',
  'create_workflow', 'list_workflows', 'update_workflow', 'complete_task',
  'create_automation', 'update_automation', 'list_automations',
  'envelope_add', 'envelope_remove', 'envelope_list',
]);

export interface CycloneToolActivityItem {
  tool: string;
  status?: string;
  args?: Record<string, unknown>;
}

export interface CycloneToolActivityPart<T extends CycloneToolActivityItem> {
  kind: 'single' | 'group';
  items: T[];
}

export function partitionCycloneToolActivity<T extends CycloneToolActivityItem>(items: T[]) {
  const parts: CycloneToolActivityPart<T>[] = [];
  let ordinary: T[] = [];
  const flush = () => {
    if (!ordinary.length) return;
    parts.push({ kind: ordinary.length > 1 ? 'group' : 'single', items: ordinary });
    ordinary = [];
  };
  for (const item of items) {
    if (SYSTEM_TOOLS.has(item.tool.trim().toLowerCase())) {
      flush();
      parts.push({ kind: 'single', items: [item] });
    } else {
      ordinary.push(item);
    }
  }
  flush();
  return parts;
}
