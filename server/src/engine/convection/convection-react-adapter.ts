...
import type { ReActStreamEvent } from '../shared/react/native-loop.js';
...
function defaultDependencies(params: RunReActParams): ConvectionTextDependencies {
  return {
    ...
    callAgentTool: async (name, args) => {
      const memoryResult = await executeConvectionMemoryTool(params.dataDir, params.agentId, name, args);
    ...
    llm: {
      call: (messages, options, onChunk, signal, _tools, onReasoning) => (
        dependencies.callModel(
          messages,
          { temperature: params.temperature, ...options },
          onChunk,
          signal,
          onReasoning,
        )
