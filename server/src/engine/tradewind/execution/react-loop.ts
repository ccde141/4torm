...
export interface LLMCaller {
  call(
    ...
    signal?: AbortSignal,
  ): Promise<{ content: string; finishReason: 'stop' | 'length' | null }>;
}
...
export async function runReActLoop(params: ReActLoopParams): Promise<ReActLoopResult> {
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    if (signal?.aborted) {
      return { content: '[中止]', rawContent: '', toolCalls: allToolCalls, turns: turn };
    }
    ...
    try {
      ...
      return { content: `[错误] ${msg}`, rawContent: '', toolCalls: allToolCalls, turns: turn };
    } finally {
    ...
    if (!reply.trim()) {
      ...
      if (!retryReply.trim()) {
        const hint = allToolCalls.length > 0
          ...
          : '（模型未返回内容，建议重试。）';
        return { content: hint, rawContent: '', toolCalls: allToolCalls, turns: turn + 1 };
      }
      reply = retryReply;
    }
    ...
    if (actions.length === 0 || !tools) {
      ...
      if (answer !== null) {
        nudgeCount = 0;
        return { content: answer, rawContent: reply, toolCalls: allToolCalls, turns: turn + 1 };
      }
      ...
      return { content: stripInternalTags(reply), rawContent: reply, toolCalls: allToolCalls, turns: turn + 1 };
    }
...
如果剩下确实只是 1-2 步收口工作（如生成最终回答），继续亲自做完即可，不必强行 delegate。`;
  ...
  return { content: stripInternalTags(raw), rawContent: raw, toolCalls: allToolCalls, turns: maxTurns };
}
