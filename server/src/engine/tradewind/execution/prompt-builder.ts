...
export function buildTradewindSystemPrompt(params: TradewindPromptParams): string {
  ...
  // §4 输出协议 + 工具列表
  sections.push(buildToolProtocol(params.toolDefs, params.allowDelegate, params.sandboxLevel));
...
function buildToolProtocol(toolDefs: ToolDef[], allowDelegate: boolean, sandboxLevel: SandboxLevel): string {
  // delegate 不在 registry，所以这里 filter 是兜底（理论上不会命中）
  const tools = toolDefs.filter(t => t.name !== 'delegate');
...
${toolList}${allowDelegate ? buildDelegateSection(sandboxLevel) : ''}`;
}
