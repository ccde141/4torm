...
| 运行记录 | 信风执行、潮汐历史、气旋异步派发和任务板执行记录 | 服务端运行过程 |
...
- Agent 当前是否工作以及所在功能区
- 尚未进入持久化记录的实时 token、思考过程和临时工具进度
- 当前 SSE 连接
- 尚未落盘的写入队列
- MCP 当前连接实例
- 潮汐调度计时器
...
后台命令和执行视窗的可见记录会保存在 `data/runtime/execution-observations.json`，用于刷新页面后重新显示任务板记录。实际执行句柄仍只存在于当前服务进程；程序重新启动后，先前未结束的记录会标记为异常中断，不会自动恢复命令。
...
仓库继续跟踪提供商与潮汐模板、框架工具清单、框架工具执行器和内置技能。`data/tools/registry.json` 只保存本机自定义工具并被 Git 忽略；用户创建的执行器同样默认不进入仓库
