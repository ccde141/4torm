...
浏览器可在执行表面中查看和接管。目前仅在季风会话和气旋普通工位私聊中提供；浏览器不会自动打开，用户可从任务板的执行记录进入。

## 后台执行

季风会话和气旋普通工位私聊可以将允许后台运行的工具转入后台。执行记录绑定原会话或工位，其他会话不能查看、等待或终止。

Agent 可以使用执行编号查询状态、有限等待或终止；人类可以从任务板查看输出并手动终止。已经结束的记录会保留一段时间，供原会话或工位查看。

后台执行主要保存在服务端进程内。正常关闭时，服务端会终止仍在运行的后台工具并等待其结束；强制结束进程时，尚未完成的执行结果不能恢复。
...
刷新或关闭浏览器页面不会关闭 Fastify 服务端。已经转入后台的命令、气旋派发等由服务端独立托管的任务可以继续执行；与当前页面流式连接绑定的交互回复可能因连接断开而中止

重新进入页面后，功能区会读取已经保存的消息和状态；支持活动同步的功能区还会查询由服务端继续托管的任务是否仍在运行。刷新前已经发送到浏览器的实时 token、思考过程和工具动画不会重新播放，已经保存的最终结果会重新载入

## 程序关闭与重新启动

收到正常退出信号时，服务端会停止潮汐调度和信风执行，终止仍在运行的后台工具，并等待潮汐运行、气旋派发、后台执行及原子写入队列排空，然后断开 MCP 并关闭服务。退出排空最多等待十秒
...
| 模型调用、工具协议和功能区共用机制 | `server/src/engine/shared/` |
| 工具执行、运行状态、数据存储和调度服务 | `server/src/services/` |
...
- 模型回复或功能区会话行为异常，从对应功能区的 engine 开始
- 工具执行、后台任务或执行视窗状态异常，从 `server/src/services/` 和工具路由开始
...
│       │   └── shared/               模型、工具协议与功能区共用机制
│       ├── services/                 工具执行、运行状态、存储与调度服务
│       │   └── tide/                 潮汐调度与任务运行
│       ├── config/                   开发模式与日志策略
│       ├── types/                    服务端共享类型
