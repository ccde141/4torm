...
| `context` | string | **自动生成** | 上游 input 前 500 字摘要（`summarizeEnvelope`）。通过连线 `contextMode: false` 可禁用 |
...
3. `context` = 上游 input 前 500 字（若 contextMode !== false）
...
│ context: ""         │ ──500字──→ │ context: "import url"│  ← summarizeEnvelope
...
| `contextMode` | boolean | `true` | 是否向下游携带上游 input 的前 500 字摘要 |
| `injectRole` | boolean | `false` | 是否用提取的字段值覆盖下游 role。**生效顺序：先设节点自身 role，再 apply injectRole（可覆盖节点自身 role）** |

---

## 三、内置节点类型

所有节点类型定义在 `src/types/sandbox.ts` 的 `SandboxNodeType` 联合类型中，共 14 种。
...
| `loop-count` | LoopNode | ✅ (inline) | ✅ LoopNode | ❌ 仅兼容旧工作流 |
| `loop-while` | LoopNode | ✅ (inline) | ✅ LoopNode | ✅ |
...
5. `resolvedRole` = `resolveTemplate(data.agentRole, envelope)` 解析模板变量
...
│                                                              │
│  System: buildSystemPrompt(toolDefs) + agentRole + workspace │
│  User:   buildTaskPrompt(envelope)  →  goal/context/input/   │
│          requirement                                             │
│                                                              │
│  ┌──────────────────────────────────────┐                    │
...
<answer>最终答案</answer>
（含工具列表 [必填]/[可选] 标注、规则、write_file 示例）

## 角色与任务指令                                     ← resolvedRole（agentRole 模板解析后）
{你配置的角色描述}
...
```
  ...
  System: buildAgentSystemPrompt(data, envelope)
          → role + outputSchema + requirement + reminder + 信封格式指令
...
#### 多输入口 (Multi-Port Input)

Agent 节点支持**左侧多输入口**，节点数据中默认带一个输入口（`in-0`），执行前强制校验全连接（`validateWorkflow()`）。执行引擎仍完整支持多端口合并逻辑（`mergeInputEnvelopes()`）。
...
**合并逻辑**（`mergeInputEnvelopes()`）：

```typescript
inputSections = ports.map((port, i) =>
  `## ${port.label}\n${upstreamEnvs[i]?.input ?? '(未到达)'}`
);
env.input = inputSections.join('\n\n');
env.goal = upstreamEnvs[0]?.goal || '';   // 取第一个上游的 goal
variables = upstreamEnvs 逐个 Object.assign 合并（后到覆盖）
```

**模板变量 `{{input.标签名}}` 解析**（`resolveTemplate()`）：按 `## 标签名` 在 `input` 中定位 Markdown section header，提取到下一个 `##` 之前的内容。
...
| 字段 | 键 | 类型 | 说明 |
|------|-----|------|------|
| 节点名称 | `label` | text | 显示在画布上 |
| Agent ID | `agentId` | select（下拉） | 关联沙盒中已添加的 Agent |
...
| `{{context}}` | envelope.context | 上游输出前 500 字 |
| `{{input}}` | envelope.input | 当前输入（多输入口时为结构化拼接） |
| `{{input.标签名}}` | 按 `## label` 提取 | 多输入口时按 label 提取单个端口内容 |
...
### 3.4 Loop-Count — 次数循环（仅兼容，不可新建）

> **已废弃：** 面板上不再提供新建入口。仅对历史工作流中的已有 `loop-count` 节点保留执行兼容（`executor.ts` inline handler）和画布渲染（`FlowCanvas.tsx` → `LoopNode` 组件）。新工作流请使用 **Loop-While**。

固定次数迭代，不调用 LLM。

**执行逻辑**（主循环 inline，无 `register`）：
```
data.count 次循环：
  → createEnvelope({ iteration: i })
  → 执行 successors 链（子节点）
  → 换轮
```
...
|------|-----|------|------|
| 节点名称 | `label` | text | |
| 循环次数 | `count` | number (1-20) | |

---

### 3.5 Loop-While — 条件循环

满足条件则继续循环。**每轮在执行完整个循环体后检查条件字段**，若满足则继续下一轮。

**代码位置:** `executor.ts` 主循环 inline handler（`node.type === 'loop-while'` → lines ~325-438）

#### 双 Handle 接线

| Handle | 标签 | 连接目标 |
|--------|------|---------|
| `loop-body` | 循环体 | 循环体内部第一个节点 |
| `loop-exit` | 循环出口 | 退出后执行的第一个外部节点 |

#### 主体执行流程

```
1. 从上游创建初始信封（lastEnv）
2. 从 bodyEdge 出发 BFS，发现所有循环体子图节点（bodyNodes）
3. 从 exitEdge 出发 BFS，发现所有出口路径节点（exitNodes = body path 排除集）
4. for 每轮迭代 (max 20):
     a. ctx.signal 中断? → break
     b. 清空 bodyNodes 的 completed/envelopes
     c. 并行执行所有 bodyNodes（拓扑顺序，executeNode 自动处理依赖）
     d. 最后一轮 lastEnv 赋值给 envelopes[nodeId]
     e. 条件检查: extractFromEnvelope(lastEnv, conditionField)
        → evaluateCondition(fieldValue, conditionOperator, conditionValue)
        → 不满足? → break
5. completed.add(nodeId)，主循环继续（exitEdge 目标由主循环拓扑调度）
```

**条件检查时机：**
- 第 0 轮：不检查条件，首轮始终执行
- 第 1 轮起：body 拓扑执行完成后，用最后一轮 body 末端节点的 envelope 提取条件字段值判断

**接线规则：**
1. 循环体内的节点最终必须通过连线回到 Loop-While 自身（构成闭环）
2. `loop-exit` handle 连到退出循环后继续执行的节点
3. 不要在循环体内部连接 `loop-exit`
4. 循环体内可以嵌套 Condition 节点（路由在每轮迭代中正常生效）

**配置:**
| 字段 | 键 | 类型 | 说明 |
|------|-----|------|------|
| 节点名称 | `label` | text | |
| 条件字段 | `conditionField` | text | 从信封中提取的字段名 |
| 条件运算符 | `conditionOperator` | select | eq / neq / gt / gte / lt / lte / regex / expr |
| 条件值 | `conditionValue` | text | 条件阈值 |
| 最大迭代 | `maxIterations` | number (1-20) | 安全上限 |
...
→ SandboxPage.tsx 中 humanGate dialog 弹出
...
将**完整执行报告**（含每个节点的输入/输出/日志）写入文件。**执行完毕后修改 envelope.input 为完整报告。**

**代码位置:** `executor.ts` → `register('output', ...)`

**输出内容包含：**
- **执行报告（Markdown）**：`buildExecutionReport()` 逐节点列出 goal / role / input
- **最终输出**：最后节点的 input
...
- `xml`: `serializeEnvelope(envelope)` 完整 XML
- `txt`: Markdown 报告
...
### 3.12 Subflow — 子流程（Stub）

预留的子工作流调用节点，**当前为占位透传**（仅前缀标记，不实际递归执行）。

**代码位置:** `executor.ts` → `register('subflow', ...)`

```
envelope.input = `[子流程 {subflowName} 输出]\n${envelope.input}`
```
...
4. 拓扑主循环（外层 while progress && !aborted）：
   for nodeId in inDegree:
     ...
     c. 特殊节点分流：
        ...
        - loop-while: inline handler（BFS 发现 body/exits，完整子图拓扑执行 N 轮）
        - loop-count: inline handler（固定次数执行 successors）
...
| Agent 多输入口 (inputPorts > 1) | 收集各端口上游 → `mergeInputEnvelopes(portEnvelopes, ports)` |
...
| `{{input}}` | `envelope.input` | 当前输入（多输入口时为完整拼接） |
| `{{input.标签名}}` | 从 input 按 `## 标签名` 提取 section | 多输入口独立引用 |
...
### Plan-and-Execute（无循环）

```
Entry → Agent(Executor) → Agent(Planner) → Output
```

### Planner 判断循环（Loop-While）

```
Entry → Plan-Agent → Variable(write) → Execution-Agent → Condition
         ↑                                                    │
         │                                          output-0 (done) → Loop-While (exit) → Output
         │                                              ↕
         │                                    output-default → Loop-While (body) → (回到 Plan-Agent)
         └────────────────────────────────────────────────────────────┘
```
...
> 更新于 2026-04-30：同步代码审计后修正（P0/P1/P2 修复 + 死代码清理后实际逻辑）。
